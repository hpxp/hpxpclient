...
_HORCM_OPT_NAMES = ['hpxp_horcm_enable_resource_group']
_HORCM_WAITTIME = 1
...
_HORCM_VOLUME_OPTS = [
    ...
    cfg.BoolOpt(
        'hpxp_horcm_enable_resource_group',
        default=False,
        secret=True,
...
class HPXPHORCM(common_ext.HPXPCommonExtension):
    ...
    def _retry_login(self, ignore_enauth, do_login):
        if not ignore_enauth:
            ...
            if do_login or result[0]:
                utils.output_log(323, user=self.conf.hpxp_horcm_user)
                return False
    ...
    def _restart_horcmgr(self, horcmgr):
        ...
        if not loop.start(interval=_HORCM_WAITTIME).wait():
            msg = utils.output_log(
                608, inst=self.conf.hpxp_horcm_numbers[horcmgr])
    ...
    def _create_thin_copy_pair(self, pvol, svol, dummy_vol_type):
        ...
        self.run_raidcom(
            ...
            self.conf.hpxp_thin_pool, '-snapshot_name',
            snapshot_name, '-copy_size', self.conf.hpxp_copy_speed)
        try:
    ...
    def connect_storage(self):
        ...
        if self.conf.hpxp_horcm_add_conf:
        ...
        super(HPXPHORCM, self).connect_storage()
...
""" % port
    ...
    def config_lock(self):
        if not CONF.hpxp_horcm_enable_resource_group:
            ...
            storage = '_'.join(
                [self.conf.hpxp_storage_id,
                 self.conf.hpxp_horcm_resource_name])
        for key in ['create_ldev', 'create_pair']:
            self.lock[key] = '_'.join([key, storage])
        self.lock[_HORCMGR] = (
            'horcmgr_%s' % self.conf.hpxp_horcm_numbers[_HORCMGR])
